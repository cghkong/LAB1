...
public class TurtleSoup {
    ...
    public static Set<Point> convexHull(Set<Point> points) {
        ...
        Point pa = new Point(Double.MAX_VALUE, Double.MAX_VALUE);
        for (Point i : points) {
            if (i.x() < pa.x() || (i.x() == pa.x() && i.y() < pa.y()))
                pa = i;
        }
        Point curPoint = pa, minPoint = null, lastPoint = pa;
        double x1 = 0.0, y1 = -1.0;
        do {
            convexHullPoints.add(curPoint);
            double minTheta = Double.MAX_VALUE, x2 = 0.0, y2 = 0.0;
            for (Point i : points) {
                if ((!convexHullPoints.contains(i) || i == pa) && (i != lastPoint)) {
                    double x3 = i.x() - curPoint.x(), y3 = i.y() - curPoint.y();
                    double Ta = Math
                            .acos((x1 * x3 + y1 * y3) / Math.sqrt(x1 * x1 + y1 * y1) / Math.sqrt(x3 * x3 + y3 * y3));
                    
                    if (Ta < minTheta || (Ta == minTheta && x3 * x3 + y3 * y3 > Math.pow(i.x() - minPoint.x(), 2)
